...
static loaderImports_t li; // loader 
...
void GVM_InitGame( int levelTime, int randomSeed, int restart ) {
	if (li.InitGame) {
		li.InitGame(levelTime, randomSeed, restart, gvm->dllHandle);
	}
...
char *GVM_ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	char* loaderOut = NULL;
	if (li.ClientConnect) {
		if ((loaderOut = li.ClientConnect(clientNum, firstTime, isBot))) {
			return loaderOut;
		}
	}
	if ( gvm->isLegacy )
...
void GVM_ClientBegin( int clientNum ) {
	if (li.ClientBegin) {
		li.ClientBegin(clientNum, qtrue);
	}
	if ( gvm->isLegacy ) {
...
qboolean GVM_ClientUserinfoChanged( int clientNum ) {
	qboolean loaderOut = qfalse;
	if (li.ClientUserinfoChanged) {
		if ((loaderOut = li.ClientUserinfoChanged(clientNum))) {
			return loaderOut;
		}
	}
	if ( gvm->isLegacy )
...
void GVM_ClientDisconnect( int clientNum ) {
	if (li.ClientDisconnect) {
		li.ClientDisconnect(clientNum);
	}
	if ( gvm->isLegacy ) {
...
static void SV_LocateGameData( sharedEntity_t *gEnts, int numGEntities, int sizeofGEntity_t, playerState_t *clients, int sizeofGameClient ) {
	...
	if (li.GameDataLocationAcquired) {
		li.GameDataLocationAcquired(gEnts, numGEntities, sizeofGEntity_t, clients, sizeofGameClient);
	}
}
...
extern "C" Q_EXPORT gameImport_t* QDECL SV_BindLoader(loaderImports_t* loaderImports) {

	static gameImport_t gi;


	memset(&li, 0, sizeof(loaderImports_t));

	li.InitGame = loaderImports->InitGame;
	li.ShutdownGame = loaderImports->ShutdownGame;
	li.ClientConnect = loaderImports->ClientConnect;
	li.ClientUserinfoChanged = loaderImports->ClientUserinfoChanged;
	li.ClientDisconnect = loaderImports->ClientDisconnect;
	li.ClientCommand = loaderImports->ClientCommand;
	li.ClientThink = loaderImports->ClientThink;
	li.ConsoleCommand = loaderImports->ConsoleCommand;
	li.ClientBegin = loaderImports->ClientBegin;
	li.RunFrame = loaderImports->RunFrame;
	li.GameDataLocationAcquired = loaderImports->GameDataLocationAcquired;
